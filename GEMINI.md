...
- `src/render.py`: Pixel geometry — `Viewport`, the map/HUD/minimap rects, and the camera transform (`compute_viewport`) — plus `RenderProcessor`, the shared base for every pixel-drawing processor (`RenderSystem` + the `ui_systems`): it holds the window `Surface` and `AssetLoader` and exposes the UI `font` and `measure(text)`.
- `src/ui_draw.py`: Pixel-native drawing primitives (`blit_text`, `blit_text_right`, `blit_segments`, `bar`, `panel`/`panel_height`, `fill_alpha`, `scroll_arrows`) shared by the UI systems. `panel(w, h, title)` sizes in pixels and returns the inner content `Rect`; callers place rows at `content.y + row * LINE_H`. `ui_helpers.wrap_message(segments, max_width, measure)` is width-unit-agnostic — real callers pass `self.measure` (pixels), tests pass `len` (chars).
